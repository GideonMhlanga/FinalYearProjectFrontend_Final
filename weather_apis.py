...
class WeatherAPIs:
    """
    Class to handle weather and environmental data through RapidAPI
    """
    ...
    def _make_api_request(self, endpoint: str) -> Dict:
        ...
        try:
            url = f"https://{self.host}/city/{self.location}/EN"
    ...
    def get_solar_irradiance(self, timestamp: datetime) -> Dict[str, float]:
        """
        Get solar irradiance data from RapidAPI weather data
        ...
        """
        try:
            # Get current weather data from RapidAPI
            endpoint = f"/city/{self.latitude}/{self.longitude}"
            ...
            is_day = self._is_daytime(timestamp, data.get('sys', {}))
            
            # Calculate irradiance (W/m²)
            if is_day:
                # Clear sky irradiance adjusted for cloud cover
                clear_sky = 1000  # Maximum expected irradiance
                irradiance = max(0, clear_sky * (1 - cloud_cover / 100))
            else:
                irradiance = 0.0
            
            return {
                'ghi': irradiance,  # Global Horizontal Irradiance
                'dni': irradiance * 1.2,  # Direct Normal Irradiance (estimated)
                'dhi': irradiance * 0.3,  # Diffuse Horizontal Irradiance (estimated)
                'timestamp': timestamp
            }
    ...
    def get_wind_speed(self, timestamp: datetime) -> Dict[str, float]:
        """
        Get wind speed data from RapidAPI weather data
